...
function validarCampos($nome, $cpf, $vacina, $dose, $data1, $data2, $foto) {//Função que verifica os dados inseridos no formulário
    ...
    if ( $foto["error"] != 0 ) {//Verifica se teve erro no uploud da foto
        $msgErro = $msgErro . "Falha ao fazer upload da foto.<br>";
    }
 
    $tamanhoMaximo = 16 * 1024 * 1024;//Calcula o tamanho máximo de imagem para MediumBlob

    if ( $foto["error"] == 0 && ($foto["size"] > $tamanhoMaximo) ) {//Verifica se a foto passou do tamanho máximo definido
            $msgErro = $msgErro . "Insira um arquivo de foto menor que 16Mb.<br>";
    } 
    ...
    if ( $foto["error"] == 0 && 
        ...
        ( $foto["type"] != "image/bmp" )) ) {//Verifica a extensão do arquivo da foto
            $msgErro = $msgErro . "Extensão da foto não permitida.<br>";
    }
